...
def mnist_multiclass_loss(predictions: torch.Tensor, labels: torch.Tensor):
    ...
    amount_left_to_full_score = 1 - correct_answer_score
    return amount_left_to_full_score.mean()

class SimpleNet(nn.Module):
    def __init__(self):
        super().__init__()
        self.flatten_layer = nn.Flatten()
        self.fc1_layer = nn.Linear(in_features = 784, out_features = 256) # out_features is the hidden_size. The smaller the size, the greater the compression.
        self.relu_layer = nn.ReLU()
        self.fc2_layer = nn.Linear(in_features = 256, out_features = 10) # Classifies into 10 classes
...
# Training loop
# In each epoch, the model sees all training images
number_of_epochs = 60
for epoch in range(number_of_epochs):
    ...
    for images, labels in train_loader:
        ...
        # Forward pass
        outputs = model(images) # outputs has shape [64, 10] (it is a multiclass classification) with results in range [-1, 1]
        loss = mnist_multiclass_loss(outputs, labels)
        ...
        _, predicted_class = torch.max(outputs.data, 1) # Returns [greatest logit among the 10 classes, index of the class with greatest logit among the 10 classes] (2 Tensors)
